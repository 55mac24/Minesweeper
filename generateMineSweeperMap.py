...
class GenerateMineSweeperMap:
    def __init__(self, dimensions, numberOfMines, startingCoordinate, map):
        ...
        self.agent_map = [[MineSweeper.UNKNOWN_CELL for row in range(dimensions)] for column in range(dimensions)]

    def isMine_local_map(self, coordinate):
        (x, y) = coordinate
        return self.hidden_map[x][y] == MineSweeper.MINE_CELL
    ...
    def markAdjacentMines(self):
        for x in range(self.dimensions):
            for y in range(self.dimensions):
                if (self.hidden_map[x][y] == MineSweeper.MINE_CELL):
                    self.mines[(x, y)] = True
                else:
    ...
    def createMineSweeperMap(self):
        ...
        while minesToCreate > 0:
            ...
            if (x, y) == self.startingCoordinate:
                continue
            elif x < self.dimensions and y < self.dimensions and self.hidden_map[x][y] != MineSweeper.MINE_CELL:
                self.hidden_map[x][y] = MineSweeper.MINE_CELL
    ...
    def validate_agent_solution(self, agent_solution):
        ...
        for x in range(self.dimensions):
            for y in range(self.dimensions):
                if not ((self.hidden_map[x][y] == MineSweeper.MINE_CELL and agent_solution[x][y] == MineSweeper.FLAG)
                        or self.hidden_map[x][y] == agent_solution[x][y]):
                    failure = failure + 1
                if self.hidden_map[x][y] == MineSweeper.MINE_CELL and agent_solution[x][y] == MineSweeper.FLAG:
